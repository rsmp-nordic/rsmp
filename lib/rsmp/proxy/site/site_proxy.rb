...
require_relative 'status_management'
require_relative 'commands'
module RSMP
  class SiteProxy < Proxy
    include Components
    include SiteProxyExtensions::StatusHelpers
    include SiteProxyExtensions::AggregatedStatus
    include SiteProxyExtensions::Alarms
    include SiteProxyExtensions::StatusManagement
    include SiteProxyExtensions::Commands
    ...
    def acknowledged_first_ingoing(message)
      send_watchdog if message.type == 'Watchdog'
    end
    ...
    def handled_by_parent?(message)
      return true if message.is_a?(CommandRequest)
      return true if message.is_a?(StatusRequest)
      return true if message.is_a?(StatusSubscribe)

      false
    end

    def ensure_message_id(options)
      options[:m_id] || RSMP::Message.make_m_id
    end
    ...
    alias set_watchdog_interval update_watchdog_interval
